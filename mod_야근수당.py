"""
야근수당내역 자동 생성기 (GUI v2.2) — 허브용 모듈
원본: 야근수당_생성기_GUI.py
변경: Toplevel 창으로 동작하도록 수정 (quit → destroy)
"""
...
class OvertimeApp:
    ...
    def _build_ui(self):
        ...
        self.lbl_target = tk.Label(frm3, text="기존 파일: (드래그 가능)", font=self.fn_label)
        ...
        # quit → destroy (Toplevel 용)
        tk.Button(frm5, text="닫기", font=self.fn_btn,
                  command=self.root.destroy, width=10, height=2).pack(side="right")
    ...
    def _process_one(self, entry, year, month, mode, outdir, out_path):
        ...
        if mode == "new":
            ...
            batch_idx    = self.entries.index(entry)
